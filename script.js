document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateNetworkStatus(online) {
    if (networkStatusText) {
      networkStatusText.textContent = online ? "Онлайн" : "Офлайн";
      networkStatusText.classList.toggle("offline", !online);
    }
  }
  ...
  function switchTab(tab) {
    if (!stationLists) {
      console.warn("Список станцій ще не завантажено");
      return;
    }
  ...
  function togglePlayPause() {
    if (!playPauseBtn || !audio || !stationItems || !stationItems.length) {
      console.error("playPauseBtn, audio або stationItems не готові");
      return;
    }
    if (audio.paused) {
      isPlaying = true;
  ...
  document.getElementById("tab-best").addEventListener("click", () => switchTab("best"));
  document.getElementById("tab-techno").addEventListener("click", () => switchTab("techno"));
  document.getElementById("tab-trance").addEventListener("click", () => switchTab("trance"));
  document.getElementById("tab-ukraine").addEventListener("click", () => switchTab("ukraine"));
  ...
  const eventListeners = {
    ...
    visibilitychange: () => {
      if (!document.hidden && isPlaying && navigator.onLine && stationItems?.length && currentIndex < stationItems.length) {
        audio.pause();
        audio.src = stationItems[currentIndex].dataset.value;
        tryAutoPlay();
      }
