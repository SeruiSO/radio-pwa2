...
if ("serviceWorker" in navigator) {
  ...
  navigator.serviceWorker.addEventListener("message", event => {
    if (event.data.type === "NETWORK_STATUS") {
      ...
      if (event.data.online && isPlaying && stationThreads?.length && currentIndex < stationThreads.length) {
        console.log("Мережа відновлена, перезапуск відтворення");
        audio.pause();
        audio.src = stationThreads[currentIndex].dataset.value;
        tryAutoPlay();
      }
...
function tryAutoPlay() {
  ...
  if (!isPlaying || !stationThreads?.length || currentIndex >= stationThreads.length || isAutoPlaying || !audio.paused) {
...
function setupBluetoothAutoPlay() {
  if ("mediaDevices" in navigator) {
    navigator.mediaDevices.ondevicechange = async () => {
      try {
        ...
        if (hasBluetooth && stationThreads?.length && currentIndex < stationThreads.length) {
  ...
  if ("mediaSession" in navigator) {
    navigator.mediaSession.setActionHandler("play", () => {
      if (stationThreads?.length && currentIndex < stationThreads.length) {
...
function updateStationList() {
  ...
  stationThreads = stationList.querySelectorAll(".station-item");
  ...
  stationList.onclick = e => {
    ...
    if (item && !item.classList.contains("empty")) {
      currentIndex = Array.from(stationThreads).indexOf(item);
...
function changeStation(index) {
  if (index < 0 || index >= stationThreads.length || stationThreads[index].classList.contains("empty")) return;
  const item = stationThreads[index];
  stationThreads?.forEach(i => i.classList.remove("selected"));
...
window.addEventListener("online", () => {
  ...
  updateNetworkStatus("Онлайн");
  if (isPlaying && stationThreads?.length && currentIndex < stationThreads.length) {
    audio.pause();
    audio.src = stationThreads[currentIndex].dataset.value;
    tryAutoPlay();
  }
